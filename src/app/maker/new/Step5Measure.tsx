  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 pt-3 border-t border-gray-200">
        ...
        <button onClick={onNext} className="flex-1 bg-primary text-white py-3 rounded-lg font-black flex items-center justify-center gap-1">
...
function SurfaceCards({
  ...
  return (
    <div className="space-y-3">
      <MeasureCard title="Salt" subtitle="mg/m² · 기준 ≤ 50" color="bg-sky-700" icon="science" bt="Elcometer 130">
        {measurements.map(m => (
          <div key={m.zone_id} className="flex items-center gap-2 py-2 border-b border-gray-200 last:border-0">
            <div className="flex-1 font-black text-sm flex items-center gap-1">
            ...
            <input
              ...
              placeholder="—"
              className="w-24 p-2 text-lg font-black text-center border-[1.5px] border-gray-300 rounded-lg"
            />
          </div>
        ))}
      </MeasureCard>

      <MeasureCard title="Dust" subtitle="Size / Quantity · 기준 ≤ 1" color="bg-stone-700" icon="grain" bt="육안 판정" noBt>
        {measurements.map(m => (
          <div key={m.zone_id} className="flex items-center gap-2 py-2 border-b border-gray-200 last:border-0">
            <div className="flex-1 font-black text-sm flex items-center gap-1">
              <span className="material-icons text-base text-pink-700">lock</span>
            ...
            <select
              ...
              className="w-16 p-2 text-sm font-black text-center border-[1.5px] border-gray-300 rounded-lg"
            >
              <option value="">-</option>
      ...
      <MeasureCard title="Profile" subtitle="㎛ · 기준 30~75" color="bg-teal-700" icon="straighten" bt="Elcometer 224">
        {measurements.map(m => (
          <div key={m.zone_id} className="flex items-center gap-2 py-2 border-b border-gray-200 last:border-0">
            <div className="flex-1 font-black text-sm flex items-center gap-1">
              <span className="material-icons text-base text-pink-700">lock</span>
...
function DftCards({
  ...
  return (
    <MeasureCard title="DFT" subtitle="㎛ · 평균/최소/최대/측정횟수" color="bg-primary" icon="straighten" bt="Elcometer 456">
      {measurements.map(m => (
  ...
  return (
    <div className="text-center">
      ...
      <input
        ...
        placeholder="—"
        className="w-full p-2 text-base font-black text-center border-[1.5px] border-gray-300 rounded-lg"
      />
    </div>
  )
}
...
function MeasureCard({
  title, subtitle, color, icon, bt, noBt, children,
}: {
  title: string
  subtitle: string
  color: string
      ...
      <div className={`${color} text-white px-4 py-3 flex items-center justify-between`}>
        ...
        {noBt ? (
          ...
          <button
            ...
            className="bg-white text-gray-900 px-3 py-1.5 rounded-full text-xs font-black flex items-center gap-1"
          >
            <span className="material-icons text-sm">bluetooth</span>{bt}
          </button>
        )}
      </div>
